...
document.addEventListener("DOMContentLoaded", async function () {
  try {
    const responseArtists = await fetch('https://marketplacenft.azurewebsites.net/api/Artist');
    const artistData = await responseArtists.json();

    const responseNFTs = await fetch('https://marketplacenft.azurewebsites.net/api/NFT');
    ...
    for (let i = 0; i < artistData.length; i++) {
      ...
      divItem.innerHTML = `
        ...
        <div class="table_name rankings_table_artist">
          <img src="https://marketplacenft.azurewebsites.net${artistImage}" alt="" />
          <p>${artistName}</p>
        </div>
