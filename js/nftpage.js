document.addEventListener("DOMContentLoaded", async function () {
  ...
  try {
    ...
    const responseNFT = await fetch(`https://marketplacenft.azurewebsites.net/api/NFT/${nftidParam}`, {
      headers: { 'Content-Type': 'application/json' }
    });
    const nftData = await responseNFT.json();

    const responseArtist = await fetch(`https://marketplacenft.azurewebsites.net/api/Artist/${artistIdParam}`, {
      headers: { 'Content-Type': 'application/json' }
    });
    const artistData = await responseArtist.json();

    const responseAllNFTs = await fetch('https://marketplacenft.azurewebsites.net/api/NFT');
    const allNFTsData = await responseAllNFTs.json();

    const responseAllArtists = await fetch('https://marketplacenft.azurewebsites.net/api/Artist');
    ...
    for (let i = 0; i < numberOfItems; i++) {
      ...
      placeholderElement.style.backgroundImage = `url(https://marketplacenft.azurewebsites.net${nftData.imageUrl})`;
            ...
            <div class="atropos-rotate">
              <div class="atropos-inner">
                <a href="nftpage.html?id=${allId}&artist=${allArtistId}" class="more_card" onclick="passNftId(${allId})">
                  <img class="cardrow_img" src="https://marketplacenft.azurewebsites.net${allImageUrl}" />
                  <div class="nft_card_placeholder">
                    ...
                    <div data-atropos-offset="5" class="more_card_artistcard">
                      <img class="img-artist-cardrow" src="https://marketplacenft.azurewebsites.net${allArtstsImages}" alt="${allArtistsName}" />
        ...
        <div class="nftartistinfo_inner">
          ...
          <div class="nftartistinfo_artistcard">
            ...
            <a href="artistpage.html?artistId=${nftArtistId}" class="nftartistinfo_card hvr-shrink">
              <img class="img-artist-cardrow" src="https://marketplacenft.azurewebsites.net${artistImage}" alt="${artistName}">
